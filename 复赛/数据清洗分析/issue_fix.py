...
output_file = r"D:\桌面\outfiler\sss.csv"  # 替换为保存结果的路径
...
rdd.map(lambda x: ...)
    ...
    if not before_cutoff.empty:
        # 计算累计值
        total_created = before_cutoff['issues_created'].sum()
        total_closed = before_cutoff['issues_closed'].sum()
        ...
        if not march_31_record.empty:
            ...
            df.loc[idx, 'issues_created'] += total_created
            df.loc[idx, 'issues_closed'] += total_closed
        else:
            # 添加一条新记录
            new_row = {
                'id': None,  # 占位，稍后重新生成
